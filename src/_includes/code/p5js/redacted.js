...
const redactionThreshold = 70; // percent
let words = '';
...
function draw() {
	...
	for (let i = 0; i < words.length; ++i) {
		let redactWord = random(0, 100);

		const word = words[i].trim();
		const wordWidth = textWidth(word);
		
		if ((x + wordWidth) > canvasX) {
			// reset to new line
			x = leftBorder;
			y += lineSpacing;
		}

		if (redactWord >= redactionThreshold) {
			fill(51);
			rect(x, y - (lineSpacing / 2), wordWidth, lineSpacing);
		} else {
			text(word, x, y);
		}

		x += wordSpacing + wordWidth;
	}
}
