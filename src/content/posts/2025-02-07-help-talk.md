...
Make notes of what you have tried
...
Look at logs and annotate with what was happening before and after ?

List out what your assumptions are
List out ways that you could check those assumptions

Keep that list and add to it as you go.
As you proceed you can add new questions and new knowledge
...
Debugging feels hard because we take code writing skills with the degree of certainty 
that we have there and try to apply those skills into a new context. Context where we don't know what is going on and there is 
uncertainty. We need to understand that we don't understand the behaviour. Obvious but it adds stress if we don't acknowledge it. 

Stress comes because we think we should be like these gods of youtube who never make a mistake. We feel the need to get this done fast.
Speed is your enemy here. Especially with hard to find bugs. 

Not understanding is normal - and we should acknowledge that. Internal monologue is screaming at you to speed up is when we most need 
to slow down.

Progress mode -> investigation mode focus

Most common strategy:

Change a thing - did it work? Awesome, let's not ask any more questions. 
No it didn't work -> Change something else ?

10 - Change A thing
20 - Did it work?
30 - Yay!

Can work if we really understand the code that we are working on - might be fastest - possibly useful
If we don't fully understand, this can lead to a while(1) loop

The less we understand, the more likely it is that something we aren't considering is causing the problem. Strategy becomes weak!

Focus on strategies that help us find the problem - invalid or inaccurate assumptions we are making about this code. Where are we wrong? Precisely

Most of the time, it's our understanding that is wrong rather than the code. Difficult to find it if it's in our understanding yet we are looking in the code

Hard for us to detect when we are making an assumption. Things we are taking for granted. That thing has to be there by this point. Have we checked? No, then slow down and check that it actually is there. Is it the truth or is it an assumption?

Stop create a list of assumptions and use instruments - automated tests, manual runs, breakpoints, prints, logs, changing small things (psychic debugging - make a change predict how it will affect the program, see if it does break in that way) - to test those assumptions and think of others that we might be making

Bugs hide in the stuff we are not checking. What is the truth and what is our perspective. Reconsider the perspective

Write stuff down - what you have done, what you think you should check - doesn't need to be super complex and can be added to as you go along - helps if you think of something else while you are setting up another check. Seems tedious until nothing is still working and we can go back and review what we have done and what must be left.
