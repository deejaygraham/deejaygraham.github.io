...
The less we understand, the more likely it is that something we aren't considering is causing the problem. This strategy becomes weak!
...
Assumptions are hard for us to detect - these are the things we are taking for granted by definitions. That thing has to be there by this point. Have we checked? No, then slow down and check that it actually is there. Is it the truth or is it an assumption?
